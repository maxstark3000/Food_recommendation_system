...
def recommend_food(excel_file, calories_prompt_per100=None, ingredient_prompt=None, user_type_prompt=None, taste_prompt=None, negative_prompt=None, top_n=5, desired_calories=None):
    """
    Recommends food from an Excel file, sorted by score, randomized within the highest score group, and calculates serving size.
    """
...
# User input prompts
user_ingredient_prompt = input("Enter preferred ingredients (e.g., beef, cheese): ")
user_user_type_prompt = input("Enter your user type (e.g., athlete): ")
user_taste_prompt = input("Enter preferred tastes (e.g., rich): ")
negative_taste = input("Enter tastes to avoid (e.g., tender): ")
user_negative_prompt = {'Taste': negative_taste} if negative_taste else None
user_desired_calories = input("Enter desired calories per serving (or leave blank): ")
user_desired_calories = int(user_desired_calories) if user_desired_calories else None

# Call the recommendation function with user inputs
